 ...
 * 물론 특정 API 에서는 컴파일 시에 예외를 처리하지 않으면 컴파일되지
 * 않는 예외도 있습니다.
 *   
 ...
 * 여기에 자식으로는 Error 와 Exception 이 있습니다.
 * Error 의 하위 타입은 일반적으로 자바 실행기 즉, VM 에 관련된
 * 에러 상황들을 정의한 클래스 입니다.
 * 
 ...
 * 무시합니다. (처리할 수 없습니다.)
 * 
 * Exception 이란 클래스의 하위 타입이 바로 프로그래머가 처리해야 할 
 * 예외 타입들 입니다.
 * 따라서 보통 예외다 라고 표현할 때 최고 클래스는 Exception 클래스 입니다.
 * Throwable 클래스는 자식으로 Error 를 가지고 있기 때문에
